...
def draw_pictures(fashion_mnist_pictures):
    for picture in fashion_mnist_pictures:
        plt.figure()
        plt.imshow(picture)
    plt.show()
...
class FashionMNIST:
    def __init__(self):
        (x, y), (x_test, y_test) = datasets.fashion_mnist.load_data()  # 加载 MNIST 数据集
        self.train_data = [x, y]
    ...
    def get_sorted_dataset(self):
        ...
        sorted_fashion_mnist_x, sorted_fashion_mnist_y = Tools.generate_sorted_dataset(x, y, 60000)
        return sorted_fashion_mnist_x, sorted_fashion_mnist_y
